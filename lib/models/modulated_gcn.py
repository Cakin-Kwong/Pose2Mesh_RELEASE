...
class ModulatedGCN(nn.Module):
    def __init__(self, adj, hid_dim, coords_dim=(2, 3), num_layers=4, nodes_group=None, p_dropout=None, pretrained=False):
...
def build_adj(joint_num, skeleton, flip_pairs):
    adj_matrix = torch.zeros((joint_num, joint_num), dtype=torch.float)
...
def get_model(joint_num, skeleton, flip_pairs, hid_dim, coords_dim=(2, 3), num_layers=4, nodes_group=None, p_dropout=None, pretrained=False):
    joint_adj = build_adj(joint_num, skeleton, flip_pairs)
    model = ModulatedGCN(joint_adj, hid_dim, coords_dim, num_layers, nodes_group, p_dropout, pretrained)
    return model